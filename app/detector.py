...
class TrafficSignDetector:
    def __init__(self, config: InferenceConfig):
        ...
        self.classes_map_1 = self._load_json(
            base_dir / "classes" / "classes_signs.json")
        self.classes_map_2 = self._load_json(
            base_dir / "classes" / "classes_traffic_lights.json")
    ...
    def _load_json(self, path):
        if Path(path).exists():
            with open(path, "r", encoding="utf-8") as f:
                return json.load(f)
        print(f"Файл классов {path} не найден!")
        return {}
    ...
    def _process_net(self, net, classes_map, frame, w_img, h_img, color):
        ...
        indices = cv2.dnn.NMSBoxes(
            boxes, confidences, self.config.conf_threshold, self.config.iou_threshold)

        final_detections = []
        if len(indices) > 0:
            indices = [int() for i in indices]
            for i in indices:
    ...
    def detect_and_draw(self, frame, mode="both"):
        ...
        if mode in ["model1", "both"]:
            detections.extend(self._process_net(
                self.net1, self.classes_map_1, frame, w_img, h_img, color=(0, 255, 0)))

        if mode in ["model2", "both"]:
            detections.extend(self._process_net(
                self.net2, self.classes_map_2, frame, w_img, h_img, color=(0, 145, 255)))
